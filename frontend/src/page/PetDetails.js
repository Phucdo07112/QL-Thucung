...
const PetDetails = () => {
  ...
  useEffect(() => {
    const orderRedux = order?.orderItems?.find(
      (item) => item.product === petDetail?._id
    );
    if (
      orderRedux?.amount + numProduct <= orderRedux?.countInstock ||
      (!orderRedux && petDetail?.countInStock > 0)
    ) {
      setErrorLimitOrder(false);
    } else if (petDetail?.countInStock === 0) {
  ...
  const handleAddOrderProduct = () => {
    if (!user?.id) {
      ...
      console.log("orderRedux",petDetail);
      if (
        orderRedux?.amount + numProduct <= orderRedux?.countInstock ||
        (!orderRedux && petDetail?.countInStock > 0)
      ) {
        dispatch(
          addOrderProduct({
            orderPetItem: {
              ...
              pet: petDetail?._id,
              discount: petDetail?.discount,
              countInstock: petDetail?.countInStock,
            },
          })
        );
      } else {
  ...
  return (
    <div className="pb-10 bg-white">
      ...
      <div className="container">
        {petDetail && (
          <div className="flex gap-5 pt-8" key={petDetail?._id}>
                ...
                <div className="flex gap-1 items-center w-[120px] border border-[#ccc] rounded-lg">
                  ...
                  <button
                    ...
                    onClick={() =>
                      handleChangeCount(
                        "increase",
                        numProduct === petDetail?.countInStock
                      )
                    }
                  >
